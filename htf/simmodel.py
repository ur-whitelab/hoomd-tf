...
class SimModel(tf.keras.Model):
    ...
    def __init__(self, nneighbor_cutoff, output_forces=True, virial=False, check_nlist=False, dtype=tf.float32, name='htf-model', **kwargs):
        R'''
    ...
    def compute(self, nlist, positions, box, sample_weight):
        ...
        by subclass. It should return one or more values as a tuple. The first element is interpreted as forces (if ``output_forces=True``, default).
...
class MolSimModel(SimModel):
    ...
    def __init__(self, MN, mol_indices, nneighbor_cutoff, output_forces=True, virial=False, check_nlist=False, dtype=tf.float32, name='htf-mol-model', **kwargs):
        R'''
        ...
        super(MolSimModel, self).__init__(nneighbor_cutoff, output_forces=output_forces,
                                          virial=virial, check_nlist=check_nlist, dtype=dtype, name=name, **kwargs)
        self.MN = MN
    ...
    def mol_compute(self, nlist, positions, mol_nlist, mol_positions, box):
        ...
        :param mol_nlist: a ``mol_number x MN x NN x 4`` tensor containing the nearest
            neighbors broken out by molecule. An entry of all zeros indicates that less than ``NN`` nearest
            neighbors where present for a particular particle. The last axis 4
            dimensions are ``x,y,z`` and ``w``, which is the particle type. Particle
            type is an integer starting at 0. Note that the ``x,y,z`` values are a
            vector originating at the particle and ending at its neighbor.
        :type nlist: tensor
        :param mol_positions: a ``mol_number x MN x N x 4`` tensor of particle positions (x,y,z) and type.
        :type positions: tensor
...
def compute_nlist_forces(nlist, energy, virial=False):
    ...
    if virial:
        return tf.tuple([_add_energy(nlist_reduce, energy), _compute_virial(nlist, nlist_forces)])
    else:
