...
There is a workaround (:py:meth:`simmodel.SimModel.safe_norm`) in Hoomd-TF. There
is almost no performance penalty, so it is fine to replace ``tf.norm``
with :py:meth:`simmodel.SimModel.safe_norm` throughout. This method adds a small
