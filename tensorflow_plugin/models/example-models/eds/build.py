...
def make_eds_graph(N, NN, directory, cv_op, set_point):
    ...
    gradient=tf.Variable(0.1,name='gradient')                        #gradient for Gradient Descend algorithm
    alpha=tf.Variable(0.001,name='alpha')                            #coupling constant
    set_point=tf.Variable(set_point,name='set_point')                #reference value or the bias reference
    ...
    aver_r,cv_sq=cv_op(graph,N)                                      #this outputs the mean cv, cv^2 and a print 
    update_colvar_op = colvar.assign(aver_r)                         #appending the new cv to the colvar variable
    update_cv_op=colvar_sq.assign(cv_sq)                             #appedning the new cv^2 to the colvar_sq variable
    ...
    bias_potential=tf.reduce_sum((graph.safe_div((alpha_val*aver_r),set_point))) #computing the potential energy due to the bias
    ...
    graph.save(model_directory=directory, out_nodes=[update_step_op,update_colvar_op,update_cv_op,update_mean_op,print_op3,update_norm_gradient,update_alpha_op],force_tensor=forces,virial=None)
