...
def make_eds_graph(N, NN, directory, cv_op, set_point):
    ...
    aver_r,cv_sq,print_op_r,e,f=cv_op(graph,N)          #this outputs the mean cv, cv^2 and a print statement
    ...
    graph.save(model_directory=directory, out_nodes=[update_step_op,update_colvar_op,update_cv_op,print_op2,update_mean_op,print_op3,update_norm_gradient,update_alpha_op,print_op_mean,print_op_r,e,f],force_tensor=forces,virial=None)
...
def avg_r(graph,N):
    ...
    rs= graph.safe_norm(tf.math.subtract(graph.positions[:,:2],com),axis=1)#,tf.reshape(tf.tile(com,[N]),(N,2))),axis=1)



    
    real=tf.reduce_mean(rs)
    a=tf.print('rs not reduce rs',rs,output_stream=sys.stdout)
    b=tf.print('rs not reduce real ',real,output_stream=sys.stdout)
    c=tf.print('rs not reduce com',com,output_stream=sys.stdout)
    
    return real,real**2,a,b,c
