...
N, NN, N_hidden = 64, 63, 63#set NN = n_hidden for simplicity
def make_train_graph(N, NN, N_hidden):
    ...
    input_layer = multilayer_perceptron_layer(r, weights['h1'], biases['b1'], keep_prob)
    ...
    graph.save(model_directory='/tmp/ann-training', out_nodes=[printer, check, optimizer, histo, histo2])

def make_force_graph(N, NN, N_hidden):
...
make_train_graph(N, NN, N_hidden)
make_force_graph(N, NN, N_hidden)
