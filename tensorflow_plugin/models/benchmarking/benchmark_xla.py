...
def perceptron_layer(x, weights, biases, keep_prob=1.0, final_layer = False, activation_function = lambda x: x):
...
def make_weights_and_biases(N_nodes, N_layers):
    ...
    weights = {}
    biases = {}
    weights['w0']= tf.Variable(tf.ones([1, N_nodes]), name='weight_w0', trainable=True)#tf.Variable(1.0, name='weight_0')
    biases['b0']= tf.Variable(tf.zeros([N_nodes]), name='bias_b0', trainable=True)#tf.Variable(0.0,name='bias_0')
    for i in range(N_layers):
        weights['w{}'.format(i+1)]= tf.Variable(tf.ones([N_nodes, N_nodes]), name='weight_w{}'.format(i+1), trainable=True)#tf.Variable(1.0, name='weight_0')
        biases['b{}'.format(i+1)]= tf.Variable(tf.zeros([N_nodes]), name='bias_b{}'.format(i+1), trainable=True)#tf.Variable(0.0,name='bias_0')
    weights['out']= tf.Variable(tf.ones([N_nodes, 1]), name='weight_out', trainable=True)#1.0, name='weight_out')
    biases['out']= tf.Variable(tf.zeros([1]), name='bias_out', trainable=True)#0.0,name='bias_out')
    return(weights, biases)

def build_neural_network(x, weights, biases, keep_prob, N_nodes, N_layers, activation_func = lambda x: x):
...
with hoomd.tensorflow_plugin.tfcompute(model_dir,
    ...
    tfcompute.attach(nlist, r_cut=rcut, save_period=100, period=100, feed_dict=dict({'keep_prob:0': 0.8}))
    ...
    benchmark_results = hoomd.benchmark.series(warmup=6000, repeat=5,steps=50000, limit_hours=2)
