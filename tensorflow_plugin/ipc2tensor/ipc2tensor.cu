...
template<typename T>
void IPC2TFunctor<GPUDevice, T, cudaIPC_t<T> >::operator()(const GPUDevice& d, int size, void* input, cudaIPC_t<T>& ipc_memory, T* out) {
    if(!(ipc_memory.array)) {
      // TODO: Learn TF way to handle cuda errors
      auto ipc_handle = reinterpret_cast<InputMem_t*> (input);
      cudaIpcOpenMemHandle((void**) (&(ipc_memory.array)), ipc_handle->mem_handle, cudaIpcMemLazyEnablePeerAccess);
      cudaIpcOpenEventHandle(&(ipc_memory.event), ipc_handle->event_handle);
    }
    cudaEventSynchronize(ipc_memory.event);
    cudaMemcpy((void *) (out), (const void*) (ipc_memory.array), size * sizeof(T), cudaMemcpyDeviceToDevice);
}
