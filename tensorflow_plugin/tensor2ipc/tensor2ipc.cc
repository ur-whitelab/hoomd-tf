    ...
    .SetShapeFn([](shape_inference::InferenceContext* c) {
      ...
      if (!result || size > result)
        return errors::InvalidArgument(
            "Input tensor may exceed buffer size, which is set by maxsize");
      return Status::OK();
    });
...
struct TF2IPCFunctor<CPUDevice, T> {
  void operator()(const CPUDevice& d, int size, void* out, const T* in,
                  T** ipc_memory) {
...
class TensorToIpcOp : public OpKernel {
 public:
  explicit TensorToIpcOp(OpKernelConstruction* c)
      : OpKernel(c), _ipc_memory(nullptr) {
    // get shape
  ...
  void Compute(OpKernelContext* context) override {
    ...
    TF2IPCFunctor<Device, T>()(context->eigen_device<Device>(), input.size(),
                               _output_memory, input.data(), &_ipc_memory);
  }
